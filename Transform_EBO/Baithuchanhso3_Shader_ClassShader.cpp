...
int main(void)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// xây dựng các ma trận transform
		// bước 1: khởi tạo các ma trận transform - ma trận đơn vị
		glm::mat4 trans = glm::mat4(1.0f);
		// bước 2: sử dụng các phép biến đổi để dịch chuyển matran
		// dịch xuống 0.5f
		//trans = glm::translate(trans, glm::vec3(0.0f, -0.5f, 1.0f));
		// xoay một góc .. radian
		trans = glm::rotate(trans,(GLfloat)glfwGetTime() * 200.0f, glm::vec3(0.0f, 0.0f, 1.0f));
		// scale to lên 0.8f lần
		trans = glm::scale(trans, glm::vec3(0.8f, 0.8f, 0.8f)); ///Trans=Trans*glm::vec3(0.3f, 0.3f, 0.3f)
		// bước 3: tì vị trí của uniform trong shader
		GLuint transformLocation = glGetUniformLocation(ourShader.IDProgram, "transform");
		// bước 4: sử dụng matran transform
